        ...
        ..showSnackBar(
          SnackBar(
            ...
            content: Row(
              ...
              children: const [
                Icon(Icons.error),
                Expanded(child: Padding( padding:EdgeInsets.only(left:16), child:Text("city not found") )),
              ],
            ),
          ),
        );
      ...
      if(data.loading==true){
       return Scaffold(body:
          Column(mainAxisAlignment: MainAxisAlignment.spaceEvenly,
            children: [
              Text(data.lper==true?" Give location access to app or turn on  device location or set it manually from settings"
              :"Loading Weather",style: TextStyle(fontSize: _size.height/20,
              fontWeight: FontWeight.bold),),
               Align(alignment: Alignment.bottomCenter,child:data.lper==true? TextButton(
                 style: ButtonStyle(
    foregroundColor: MaterialStateProperty.all<Color>(const Color.fromARGB(255, 135, 175, 209)),
  ),
                 onPressed:  () {
                        Navigator.pushReplacementNamed(context, '/');
                      }, child: const Text("Try Again")):const CircularProgressIndicator(color: Colors.blueGrey,strokeWidth:7  ,valueColor: AlwaysStoppedAnimation<Color>( Colors.black38 ,)),
               ) 
            ],
          ),);
            }
              ...
              child: Container(
                ...
                child:Column(
                  children: [
                    ...
                    Row(children: [
                      Expanded(
                        child: Container(height: _size.height/5.5,
                          decoration: BoxDecoration(
                          ...
                          child:Row(mainAxisAlignment: MainAxisAlignment.center,
                            ...
                            Image.network("http://openweathermap.org/img/wn/${data.dt?.weather.icon}@2x.png"),
                         SizedBox(width: _size.width/25,),
                            Column(mainAxisAlignment: MainAxisAlignment.center,
                              children: [  Text(data.dt!.weather.description.toString() ,style:  TextStyle(
                                fontSize: _size.height/29,
                                fontWeight: FontWeight.bold
                              ),),
                            SizedBox(height: _size.height/50,),
                            Text(data.dt!.city.toString() ,style: TextStyle(
                          ...
                          child: Container(
                            height: _size.height/3,
                                ...
                                child:Column(
                                  ...
                                  children: [
                                    const Icon(WeatherIcons.thermometer,color: Color.fromARGB(255, 166, 82, 76),),
                                    SizedBox(height: _size.height/22,)
                                    ,Row(
                                      mainAxisAlignment: MainAxisAlignment.center,
                                      children: [
                                        
                                        Text(data.dt!.maincl.temp.toString() ,style: TextStyle(fontSize: _size.height/9),),
                                        Padding(padding: EdgeInsets.only(top: _size.height/25)
                                        ,child: Text('℃',style: TextStyle(fontSize: _size.height/16),),)
                            ...
                            child:Column(
                              children: [
                                Row(mainAxisAlignment: MainAxisAlignment.start,
                                  children: const [ 
                                    Icon(WeatherIcons.day_windy,color: Color.fromARGB(255, 100, 131, 157),),
                                  ],
                                ),
                                SizedBox(height: _size.height/50,),
                                Text(((data.dt!.wind.speed)*3.6).ceil().toString() ,style: TextStyle(fontSize: _size.height/25,fontWeight:FontWeight.bold),),
                                Text('Km/h',style: TextStyle(fontSize: _size.height/50),)
                              ],
                            )),
                        ),
                        Expanded(
                          child: Container(
                            ...
                            child:Column(
                              children: [
                                Row(mainAxisAlignment: MainAxisAlignment.start,
                                  children: const[ 
                                    Icon(WeatherIcons.humidity,color:Color.fromARGB(255, 14, 41, 175),),
                                  ],
                                ),
                                SizedBox(height: _size.height/50,),
                                Row(mainAxisAlignment: MainAxisAlignment.center,
                                  children: [
                                    Text(data.dt!.maincl.humidity.toString() ,style: TextStyle(fontSize: _size.height/25,fontWeight:FontWeight.bold),),
                                     Text('%',style: TextStyle(fontSize: _size.height/45,fontWeight: FontWeight.w600),)
                                  ],
                                ),
